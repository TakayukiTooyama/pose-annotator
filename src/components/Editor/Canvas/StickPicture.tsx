...
import type { Marker, Position } from '@/types';
...
const StickPicture: FC<Props> = ({ positionXScale, positionYScale, markers }) => {
      ...
      {stickPoints.map((points: (number | undefined)[], idx) => {
        ...
        return (
          <Line
            key={idx}
            points={[
              points[0] / positionXScale,
              points[1] / positionYScale,
              points[2] / positionXScale,
              points[3] / positionYScale,
            ]}
            stroke='white'
          />
        );
